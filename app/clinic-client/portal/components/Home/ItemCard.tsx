    ...
    <div className="border-light-gray border-2 border-opacity-30 cursor-pointer hover:shadow-md mb-2 p-5 rounded-2xl sm:p-3 transition-all duration-150">
      {isPatient(data) ? (
        ...
        >
          <BsPersonFill
            size={48}
            className="font-bold text-secondary-dark-blue md:block sm:hidden"
          />
          <h1 className="font-bold text-base text-secondary-dark-blue truncate">
            {data.firstName}
          </h1>
          <h1 className="font-bold text-base text-secondary-dark-blue truncate">
            {data.lastName}
          ...
          <IoIosArrowForward
            size={24}
            className="font-bold justify-self-end text-secondary-dark-blue hover:text-primary-light-blue"
          />
        </div>
      ) : (
        <div
          onClick={() => handleClick('order', data.id.toString())}
          className="grid-cols-1 grid items-center gap-4 sm:grid-cols-2 sm:gap-2 md:grid-cols-[100px_150px_150px_150px_150px_1fr_24px]"
        >
          <RiListUnordered
            size={48}
            className="font-bold text-secondary-dark-blue md:block sm:hidden"
          ...
          <h1 className="font-bold text-base text-secondary-dark-blue truncate">
            {format(new Date(data.orderDate), 'yyyy-MM-dd HH:mm')}
          </h1>
          <h1 className="font-bold text-base text-secondary-dark-blue truncate">
            {data.orderState}
          </h1>
